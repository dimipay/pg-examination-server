...
export const verify = (
  ...
  try {
    if (typeof token === 'undefined') {
      throw new HttpError(401, '토큰이 필요합니다.', 'TOKEN')
    }

    const payload = jwt.verify(token, config.jwtSecret) as Payload
    
    if (payload.refresh !== refresh) {
      throw new HttpError(401, '토큰 타입이 일치하지 않습니다.', 'TOKEN')
    }

    return payload
  } catch (error) {
    if (error instanceof TokenExpiredError) {
      throw new HttpError(401, '토큰이 만료되었습니다.', 'TOKEN')
    }

    if (error instanceof JsonWebTokenError || error instanceof NotBeforeError) {
      throw new HttpError(401, '토큰이 유효하지 않습니다.', 'TOKEN')
    }

    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      throw new HttpError(401, '사용자를 확인하는데 실패했습니다.', 'TOKEN')
    }
